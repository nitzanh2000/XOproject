...
enum class Result { X_SYMBOL, O_SYMBOL, DRAW }
...
class XOGame {
    ...
    fun checkForWinner(): Result? {
        for (i in board.indices) {
            ...
            } else if (board[0][i] != EMPTY_CELL && board[0][i] == board[1][i] && board[0][i] == board[2][i]) {
        ...
        } else if (board[0][2] != EMPTY_CELL && board[0][2] == board[1][1] && board[0][2] == board[2][0]) {
            return returnResult(board[0][2])
        } else if (board.all { row -> row.all { cell -> cell != EMPTY_CELL } }) {
            return Result.DRAW
        }
